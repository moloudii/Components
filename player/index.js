    ...
    this._sources.forEach((source, index) => {
      ...
      song.innerHTML = song.innerHTML.replace(/{{\s*(.*?)\s*}}/g, (_, key) => {
        ...
        if (this._currentSrcIndex === index) {
          if (key === "icon") return "equalizer";
          if (key === "playing") return "playing";
        } else {
          if (key === "icon") return "play_arrow";
